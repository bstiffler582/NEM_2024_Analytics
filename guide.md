...
Analytics Logger is our PLC's "flight recorder"; where any program symbol or IO signal can be logged locally or remotely, at rates (up to) the PLC scan rate. Configure the Analytics Logger to publish messages to our local MQTT message broker:
```
...
Upon re-activating and running the project, you should be able to check the 'Online' tab of the stream to see that the logger is <span style="color: green">Connected</span> and samples are being issued. Make sure to start the machine back up so we have some variation in the data moving forward.
...
> Open the Target Browser. Under the TcAnalytics tab, add a connection to our local broker. There you will see live and historical data available for browsing. For now, we can test our analyses on the live data - dragging and dropping symbols right from the Target Browser into the algorithms.

- Rename the blank Network something like "MachineStatus", with algorithms to calculate the following:
    - Run time (`MAIN.eMachineState == E_MachineMode.Execute`)
    - Stop time
    - Metal count
    - Non-metal count
    - Total count
    - Sortations per minute (Moving interval)
    - Cycle time (Event timing)
...
- Add a new empty network "CylinderStatus_Clamp", targeting `MAIN.fbSeparateModule.fbClamp`, with algorithms to calculate the following:
    ...
    - Min / Max / Avg
        ...
        - Vibration
    - Vibration histogram
        - Measure the max vibration, but *only during an actuation*
        - Reset max value after each actuation
        - Feed max value into a historgram with: 10 bins, values 0-100

Add HMI controls to applicable algorithms.

**Parameterize:** You can change algorithm mappings from virtual inputs to *network inputs*. This will allow us to reuse the same analysis for the metal & non-metal cylinders. Note that we also have to option of saving the network as a *template*, which would allow us to share and reuse the logic in other projects.

- Add a new empty network "ConveyorStatus". For the three conveyors, we will have simple analysis as follows:
    - Run count (`ActVelocity != 0`)
    - Run time

Keep this one simple and just analyze all three conveyors in the same network. Don't forget to add HMI controls!

**Sources and Virtual Inputs:** You may have noticed that Workbench has create an abstraction layer between data sources and the actual analyses. This is so that input data can be seamlessly swapped without having to recreate any mappings to the algorithms. This is most commonly demonstrated when changing between "Live" and historical sources.

Create a new *substitute* source and follow the wizard. Point the new source to the Analytics binary file provider. The tool will recognize commonalities between the data sets and auto-select all identical symbols. Once available in your project, you can swap virtual input sources between "Live" and "Historic". The same functionality will be available via the Dashboard interface when we deploy the Runtime.

<a id="runtime"></a>

### 5. Analytics Runtime

On the top level "Analytics Project" node, right-click and select the "Deploy Runtime" option. This will launch a wizard which walks through the runtime configuration.
- Select a new solution path (do not use the repository directory or your Analytics project solution)
- Make sure to add the historic input source so it is selectable in the Dashboard.
- We will not be running the Runtime PLC so don't worry about target config
